...
(function(window, undefined){
    var
        ...
        _toStringMap = {
            ...
            "[object String]" : function(v) {
                return _reString.test(v) ? '"' + v.replace(_reString, function(ch) {
                    var c = _stringMap[ch];
            ...
            "[object Object]" : function(v) {
                var i, s = "{";
                for(i in v) {
                    s += (s.length > 1 ? ',' : '') + JSC.toString(i) + ':' + JSC.toString(v[i]);
                }
                return s + "}";
            },

            // representation of an array
            "[object Array]" : function(v) {
                var i, s = "[";
                for(i = 0; i < v.length; i++) {
                    s += (s.length > 1 ? ',' : '') + JSC.toString(v[i]);
                }
                return s + "]";
            },

            // representation of a function
            "[object Function]" : function(v) {
                return v.toString().replace(_reCode, '$1');
            },

            // representation of a regular expression
            "[object RegExp]" : function(v) {
                return '/' + v.source + '/' + (v.global ? 'g' : '') + (v.ignoreCase ? 'i' : '');
            }
        };

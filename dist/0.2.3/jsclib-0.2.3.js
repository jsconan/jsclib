/*!
 * JavaScript Class Library v0.2.3 (JSC v0.2.3)
 *
            ...
            if( JSC.isArray(array) ) {
                ...
                if( keys.length ) {
                    ...
                    array.sort(function(a, b){
                        // check items : need objects
                        v1 = JSC.isObject(a);
                        v2 = JSC.isObject(b);
                        if( v1 && v1 == v2 ) {
                            // both are objects, compare values by sort keys
                            for(i = 0; i < keys.length; i++) {
                                k = keys[i]; v1 = a[k]; v2 = b[k];
                                if( v1 != v2 ) {
                            ...
                            // all compared values seem to be equals
                            return 0;
                        } else {
                            // objects are placed above other values, otherwise compare values between themselves
                            return v1 ? 1 : v2 ? -1 : a > b ? 1 : a < b ? -1 : 0;
                        }
                    });
                } else {
                    // native sort...
